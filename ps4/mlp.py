...
class TwoLayerMLP(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    P = F / np.sum(F, axis=1).reshape(N, 1)  # N*C, this is softmax layer
    loss = np.mean(-np.choose(y, scores.T) + np.log(np.sum(F, axis=1)) + A) # cross entropy loss
    ...
    # output layer
    dscore = P - y_C  # [PLEASE IMPLEMENT] partial derivative of loss wrt. the logits (dL/dz)
